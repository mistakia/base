...
import { add_tags_to_entity, remove_tags_from_entity } from '#libs-server/tag/filesystem/manage-entity-tags.mjs'
...
describe('Tag Management Functions', () => {
  ...
  describe('resolve_tag_shorthand', () => {
    ...
    it('should handle mixed shorthand and full format', () => {
      const result = resolve_tag_shorthand('javascript, user:tag/custom.md, python')
      expect(result).to.deep.equal([
        'user:tag/javascript.md',
        'user:tag/custom.md',
        'user:tag/python.md'
    ...
    it('should throw error for empty input', () => {
      expect(() => resolve_tag_shorthand('')).to.throw('Tag input must be a non-empty string')
    })

    it('should throw error for null input', () => {
      expect(() => resolve_tag_shorthand(null)).to.throw('Tag input must be a non-empty string')
    })
  })
  ...
  describe('add_tags_to_entity', () => {
    it('should add tags to entity with no existing tags', async () => {
      ...
      expect(result.added_tags).to.deep.equal(['user:tag/javascript.md', 'user:tag/python.md'])
    ...
    it('should deduplicate tags within input array', async () => {
      ...
      const result = await add_tags_to_entity({
        absolute_path,
        tags_to_add: ['user:tag/javascript.md', 'user:tag/javascript.md', 'user:tag/python.md']
      })
  ...
  describe('remove_tags_from_entity', () => {
    it('should remove tags from entity', async () => {
      ...
      const entity_properties = {
        title: 'Test Entity',
        user_public_key: 'abc123',
        tags: ['user:tag/javascript.md', 'user:tag/python.md', 'user:tag/react.md']
      }
      ...
      expect(result.success).to.be.true
      expect(result.removed_tags).to.deep.equal(['user:tag/javascript.md', 'user:tag/python.md'])
    ...
    it('should handle removing non-existent tags', async () => {
      ...
      expect(result.not_found_tags).to.deep.equal(['user:tag/python.md', 'user:tag/react.md'])
      expect(result.total_tags).to.equal(1)
    })
