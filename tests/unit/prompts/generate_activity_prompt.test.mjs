...
import fs from 'fs'
import generate_activity_prompt from '#libs-server/prompts/generate_activity_prompt.mjs'
...
describe('generate_activity_prompt', () => {
  ...
  before(async () => {
    ...
    test_system_dir = await create_temp_test_directory('system_activities_test')
    test_user_dir = await create_temp_test_directory('user_activities_test')
  ...
  describe('with activity_id parameter', () => {
    ...
    it('should generate prompt for a system activity with guidelines', async () => {
      ...
      const result = await generate_activity_prompt({
        activity_id: 'system/test_activity1.md',
    ...
    it('should generate prompt for a system activity without guidelines', async () => {
      // Act
      const result = await generate_activity_prompt({
        activity_id: 'system/test_activity2.md',
        system_base_directory: test_system_dir.path,
        user_base_directory: test_user_dir.path
      })
    ...
    it('should generate prompt for a user activity', async () => {
      // Act
      const result = await generate_activity_prompt({
        activity_id: 'user/user_activity1.md',
        system_base_directory: test_system_dir.path,
        user_base_directory: test_user_dir.path
      })
      ...
      expect(result.guideline_ids).to.include('user/user_guideline1.md')
    })
