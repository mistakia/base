...
describe('load_schema_definitions_from_git', () => {
  ...
  before(async () => {
    ...
    await fs.mkdir(path.join(system_base_directory, 'system', 'schema'), {
      recursive: true
    })
    ...
    await fs.writeFile(
      path.join(system_base_directory, 'system', 'schema', 'task.md'),
      `---
...
# Task Schema Definition`
    ...
    await fs.writeFile(
      path.join(system_base_directory, 'system', 'schema', 'person.md'),
      `---
type: type_definition
name: person
properties:
...
# Person Schema Definition`
    ...
    // Commit schema files
    await execute('git add system/schema/task.md system/schema/person.md', {
      cwd: system_base_directory
    })
...
# Unknown Extension Schema`
  ...
  it('should load schema definitions from repositories', async () => {
    ...
    expect(result.task.extensions[0]).to.have.property('name', 'task_extension')
  ...
  it('should throw an error if system_repository is not provided', async () => {
    try {
      ...
      expect(error.message).to.include('system_repository is required')
    }
  })

  it('should throw an error if user_repository is not provided', async () => {
    try {
      await load_schema_definitions_from_git({
        system_repository: {}
      })
      ...
      expect(error.message).to.include('user_repository is required')
    }
  })
})
