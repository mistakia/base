...
This is a test entity for CLI testing.`
  ...
  describe('CLI help and usage', function () {
    it('should display help when --help is used', async function () {
      try {
        const { stdout } = await execute(
          `NODE_ENV=test bun ${cli_path} entity visibility --help`
        )
    ...
    it('should show error for missing command', async function () {
      try {
        await execute(`NODE_ENV=test bun ${cli_path} entity visibility`)
  ...
  describe('Setting public_read on entity files', function () {
    it('should set public_read to true on markdown entity', async function () {
      ...
      const { stdout } = await execute(
        `NODE_ENV=test bun ${cli_path} entity visibility set "${test_entity_path}" true`
      )
    ...
    it('should set public_read to false on markdown entity', async function () {
      ...
      // Then set to false
      const { stdout } = await execute(
        `NODE_ENV=test bun ${cli_path} entity visibility set "${test_entity_path}" false`
      )
  ...
  describe('Setting public_read on thread metadata', function () {
    it('should set public_read to true on thread metadata.json', async function () {
      ...
      // Set public_read to true
      const { stdout } = await execute(
        `NODE_ENV=test bun ${cli_path} entity visibility set "${metadata_path}" true`
      )
    ...
    it('should set public_read to false on thread metadata.json', async function () {
      ...
      // Then set to false
      const { stdout } = await execute(
        `NODE_ENV=test bun ${cli_path} entity visibility set "${metadata_path}" false`
      )
...
Test content for ${path.basename(entity_path)}.`
    ...
    it('should handle glob patterns for multiple files', async function () {
      ...
      const { stdout } = await execute(
        `NODE_ENV=test bun ${cli_path} entity visibility set "${pattern}" true`
      )
  ...
  describe('Dry-run functionality', function () {
    it('should preview changes without applying them in dry-run mode', async function () {
      ...
      // Run in dry-run mode
      const { stdout } = await execute(
        `NODE_ENV=test bun ${cli_path} entity visibility set "${test_entity_path}" true --dry-run`
      )
  ...
  describe('Validation and error handling', function () {
    it('should reject invalid boolean values', async function () {
      ...
      try {
        await execute(
          `NODE_ENV=test bun ${cli_path} entity visibility set "${test_entity_path}" invalid`
        )
        throw new Error('Should have failed')
      } catch (error) {
    ...
    it('should handle non-existent files gracefully', async function () {
      ...
      const { stdout } = await execute(
        `NODE_ENV=test bun ${cli_path} entity visibility set "${non_existent}" true`
      )
    ...
    it('should reject unsupported file types', async function () {
      ...
      const { stdout } = await execute(
        `NODE_ENV=test bun ${cli_path} entity visibility set "${txt_file}" true`
      )
...
Summary test content.`
  ...
  describe('Getting public_read values', function () {
    it('should show public_read for a single entity', async function () {
      ...
      const { stdout } = await execute(
        `NODE_ENV=test bun ${cli_path} entity visibility get "${test_entity_path}"`
      )
    ...
    it('should show public_read for thread metadata', async function () {
      ...
      const { stdout } = await execute(
        `NODE_ENV=test bun ${cli_path} entity visibility get "${metadata_path}"`
      )
...
Extra entity content.`
      ...
      const { stdout } = await execute(
        `NODE_ENV=test bun ${cli_path} entity visibility get "${pattern}"`
      )
