  ...
  try {
    ...
    if (verbose) {
      ...
      cursor_conversations.forEach(conv => {
        ...
        log(`  ${conv.composer_id}: ${summary.message_count} messages (${summary.duration_minutes?.toFixed(1) || 'unknown'} min)`)
      })
    }

    // Step 2: Validate conversations
    log('Step 2: Validating conversations...')
    const { valid: valid_conversations, invalid: invalid_conversations } = filter_valid_cursor_conversations(cursor_conversations)

    log(`Validation complete: ${valid_conversations.length} valid, ${invalid_conversations.length} invalid`)
    ...
    if (dry_run) {
      log('Dry run mode - would create threads for the following conversations:')
      valid_conversations.forEach(conv => {
        const summary = get_conversation_summary(conv)
        log(`  ${conv.composer_id}: ${summary.message_count} messages, ${summary.summary || 'No summary'}`)
      })
    ...
    log(`Step 3: Creating threads from ${valid_conversations.length} valid conversations...`)
    const thread_results = await create_threads_from_cursor_conversations(valid_conversations, {
      user_base_directory,
      ...options
    })
...
export const import_single_cursor_conversation = async (composer_id, options = {}) => {
  try {
    ...
    log(`Found conversation with ${conversation.messages?.length || 0} messages`)
    ...
    // Create thread
    const thread = await create_thread_from_cursor_conversation(conversation, options)
...
export const list_cursor_conversations = async (options = {}) => {
  ...
  try {
    const conversations = await read_all_cursor_conversations({ db_path: cursor_data_path })
    ...
    return conversations.map(conv => {
      ...
      if (include_summaries) {
        conv_info.summary = summary.summary
        conv_info.has_code_blocks = summary.has_code_blocks
        conv_info.model_used = summary.model_used
      }
