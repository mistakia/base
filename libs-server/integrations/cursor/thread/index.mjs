...
import { CURSOR_THREAD_NAMESPACE } from '#libs-server/integrations/cursor/cursor-config.mjs'
 ...
 */
export const create_thread_from_cursor_conversation = async (
  conversation,
  options = {}
) => {
  ...
  // Generate thread ID
  const thread_id = generate_thread_id_from_cursor_conversation(
  ...
  session.metadata.session_provider = 'cursor'
  ...
  const thread = await create_thread_from_session({
 ...
 */
export const create_threads_from_cursor_conversations = async (
  conversations,
  options = {}
) => {
  ...
  for (const conversation of conversations) {
    try {
      const result = await create_thread_from_cursor_conversation(
        conversation,
        options
      )
      ...
      log(
        `Failed to create thread for conversation ${conversation.composer_id}: ${error.message}`
      )
      results.failed.push({
        composer_id: conversation.composer_id,
        error: error.message
  ...
  log(
    `Thread creation complete: ${results.created.length} created, ${results.failed.length} failed, ${results.skipped.length} skipped`
  )
  return results
}
