...
export const find_cursor_composer_data = async ({ db_path }) => {
  const fullPath = expandPath(db_path)
  log(`Opening Cursor database at: ${fullPath}`)
...
export const read_cursor_conversation = async (composer_id, options = {}) => {
  ...
  const fullPath = expandPath(db_path)
  const key = composer_id.startsWith('composerData:') ? composer_id : `composerData:${composer_id}`

  log(`Reading conversation ${key} from ${fullPath}`)
  ...
  try {
    const row = await db.get('SELECT value FROM cursorDiskKV WHERE key = ?', key)
    if (!row) {
...
export const read_all_cursor_conversations = async ({
  ...
  const composerDataRows = await find_cursor_composer_data({ db_path })
  ...
  try {
    const rowsToProcess = limit ? composerDataRows.slice(0, limit) : composerDataRows
    ...
    for (const row of rowsToProcess) {
      try {
        ...
        const conversation = extract_conversation_data(data, row.key.replace('composerData:', ''))
...
function extract_conversation_data(data, composer_id) {
  ...
  if (data.conversation) {
    log(`Processing conversation for ${composer_id}, type: ${typeof data.conversation}, isArray: ${Array.isArray(data.conversation)}`)

    // Single conversation format - could be array or object with numeric keys
    if (Array.isArray(data.conversation)) {
      log(`Processing conversation array with ${data.conversation.length} entries`)
      conversation.messages = extract_messages_from_conversation(data.conversation)
    } else if (typeof data.conversation === 'object') {
      // Object with numeric keys - convert to array
      const conversationArray = Object.keys(data.conversation)
        .filter(key => !isNaN(key))
        .sort((a, b) => parseInt(a) - parseInt(b))
        .map(key => data.conversation[key])

      log(`Processing conversation object with ${conversationArray.length} entries`)
      conversation.messages = extract_messages_from_conversation(conversationArray)
    }
    ...
    const convIds = Object.keys(data.conversationMap)
    if (convIds.length > 0) {
      ...
      for (const convId of convIds) {
        const conv = data.conversationMap[convId]
        const convTime = conv.updatedAt || conv.createdAt || 0
      ...
      if (latestConv) {
        conversation.messages = extract_messages_from_conversation(latestConv)
      }
    }
    conversation.summary = data.latestConversationSummary
...
function extract_messages_from_conversation(conv) {
  ...
  log(`extract_messages_from_conversation called with type: ${typeof conv}, isArray: ${Array.isArray(conv)}, length: ${conv?.length}`)
  ...
  if (Array.isArray(conv)) {
    ...
    for (let i = 0; i < Math.min(conv.length, 10); i++) {
      const entry = conv[i]
      log(`Entry ${i}: type=${typeof entry}, keys=${entry ? Object.keys(entry).join(',') : 'null'}`)
...
function extract_message_data(msg) {
  ...
  const message = {
    id: msg.bubbleId || msg.id || msg.uuid || generateMessageId(),
    role,
...
export const get_conversation_summary = (conversation) => {
  ...
  return {
    ...
    end_time,
    duration_minutes,
    summary: conversation.summary || `Conversation with ${message_count} messages`,
    has_code_blocks,
    model_used
  }
}
