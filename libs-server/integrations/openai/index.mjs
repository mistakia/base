...
import { createOpenAIClient } from './api/index.mjs'
import { normalize_openai_conversation, validate_openai_session } from './normalize-session.mjs'
  ...
  try {
    ...
    if (verbose) {
      log('Options:', {
        ...
        include_archived,
        dry_run,
        has_auth: !!(bearer_token && session_cookies && device_id)
      })
    }
    ...
    for (const conv_summary of filtered_conversations) {
      try {
        const full_conversation = await client.getConversation(conv_summary.id)
        ...
        log(`✗ Failed to fetch conversation ${conv_summary.id}: ${error.message}`)
        failed_fetches.push({
          id: conv_summary.id,
    ...
    for (const conversation of full_conversations) {
      try {
        ...
        if (validation.valid) {
          ...
          if (verbose) {
            log(`✓ Normalized: ${normalized.title} (${normalized.messages.length} messages)`)
          }
        } else {
          log(`✗ Invalid session ${conversation.id}: ${validation.errors.join(', ')}`)
    ...
    log(`Results: ${thread_results.created.length} created, ${thread_results.failed.length} failed, ${thread_results.skipped.length} skipped`)
...
export async function get_openai_conversation(conversation_id, auth_options) {
  ...
  try {
    ...
    return await client.getConversation(conversation_id)
  } catch (error) {
    log(`Failed to get OpenAI conversation ${conversation_id}: ${error.message}`)
    throw error
  }
}
 ...
 */
export async function validate_openai_auth(auth_options) {
  try {
    const client = createOpenAIClient(auth_options)
