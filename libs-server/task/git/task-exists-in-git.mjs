...
const log = debug('task:exists-in-git')

/**
 * Check if a task file exists in a git branch
 *
 ...
 */
export async function task_exists_in_git({
  base_relative_path,
  branch,
  ...
  try {
    log(
      `Checking if task exists in git: ${base_relative_path} (branch: ${branch})`
    )
    ...
    log(
      `Checking task in git at path: ${git_relative_path} in repo: ${repo_path}`
    )
    ...
    if (!result.success) {
      return {
        success: false,
        error: result.error || 'Failed to check if task exists in git',
        base_relative_path,
        branch
      }
    }
    ...
    log(`Error checking if task exists in git: ${error.message}`)
    return {
      success: false,
      error: error.message,
      base_relative_path,
