...
const CURSOR_BASE_PATH = path.join(
  os.homedir(),
  'Library',
  'Application Support',
  'Cursor'
...
async function exploreCursorSQLite() {
  try {
    ...
    // Get all tables
    const tables = await globalDb.all(
      "SELECT name FROM sqlite_master WHERE type='table'"
    )
    log(`Tables: ${tables.map((t) => t.name).join(', ')}`)

    // Check cursorDiskKV table
    const cursorKVCount = await globalDb.get(
      'SELECT COUNT(*) as count FROM cursorDiskKV'
    )
    log(`cursorDiskKV rows: ${cursorKVCount.count}`)

    if (cursorKVCount.count > 0) {
      const cursorKeys = await globalDb.all(
        'SELECT key, length(value) as size FROM cursorDiskKV ORDER BY size DESC LIMIT 20'
      )
    ...
    for (const dir of workspaceDirs) {
      ...
      try {
        ...
        // Get row count
        const itemCount = await wsDb.get(
        ...
        if (chatKeys.length > 0) {
          ...
          const largeComposer = chatKeys.find(
            (k) => k.key.includes('composerChatViewPane') && k.size > 1000
          )
          if (largeComposer) {
            const data = await wsDb.get(
              'SELECT value FROM ItemTable WHERE key = ?',
              largeComposer.key
            )
            try {
              ...
              log(
                `    Sample structure: ${JSON.stringify(Object.keys(parsed), null, 2)}`
              )
    ...
    log(
      '1. workbench.panel.composerChatViewPane.* entries in workspace state.vscdb files'
    )
