...
Sync-state cache: `os.tmpdir()` + sha256(absolute `session_file` path) prefix. Per-host transient. The state shape carries `byte_offset, leaf_id, branch_thread_id, schema_version`; the cache key on the file path (rather than per-branch session id) reflects that Pi files host multiple branches that share one transcript. Crash safety: append timeline -> write metadata -> commit sync state. Re-running any tick is idempotent because dedup is by `source_entry_id`.
