...
The detector runs two cheap filters before any timeline I/O. First, a fast-path gate scans the analyzed thread's first user prompt against the continuation-signal vocabulary (see `libs-server/metadata/continuation-signal.mjs`); if no signal is present, `detect_continuation_source` returns `[]` in under a millisecond without enumerating candidates. Second, during the metadata pass, candidates whose `metadata.has_continuation_prompt === false` are dropped without reading their timeline; candidates where the flag is missing pass through and fall back to an in-pass timeline-read scan that applies the same vocabulary. The analyzer also caches `metadata.has_continuation_prompt` and `metadata.continuation_prompt_count` on each thread it writes so future runs can use the metadata-only prefilter; a one-shot `cli/backfill-continuation-signal.mjs` script populates both fields across existing threads.
</instructions>
