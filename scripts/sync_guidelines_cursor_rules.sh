...
find_guideline_files() {
    local system_files=$(find "$SYSTEM_GUIDELINES_DIR" -type f -name "*.md" -exec basename {} .md \; | sed 's/^/system_/' | sort)
    local data_files=$(find "$DATA_GUIDELINES_DIR" -type f -name "*.md" -exec basename {} .md \; | sed 's/^/user_/' | sort)
    echo -e "$system_files\n$data_files"
}
...
sync_guidelines "$SYSTEM_GUIDELINES_DIR" "system_"
system_changes=$?

sync_guidelines "$DATA_GUIDELINES_DIR" "user_"
data_changes=$?
